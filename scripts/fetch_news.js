...
const NEWS_IMAGES_DIR = path.join(__dirname, '../images/news');
...
async function downloadMedia(url, filename) {
    if (!url) return null;
    try {
        const localPath = path.join(NEWS_IMAGES_DIR, filename);
        if (fs.existsSync(localPath)) return `images/news/${filename}`;
...
async function fetchNews() {
    ...
    for (const item of uniqueNews) {
        ...
        if (item.externalImage && item.externalImage.startsWith('http')) {
            ...
            const localPath = await downloadMedia(item.externalImage, filename);
        ...
        if (item.externalVideo && item.externalVideo.startsWith('http')) {
            ...
            const localPath = await downloadMedia(item.externalVideo, filename);
